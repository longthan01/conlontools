...
class Inbox {
    ...
    async getEmail(id) {
        ...
        try {
            ...
            throw new Error("Could not retrieve email. Inner error: " +
                ((_a = err.message) !== null && _a !== void 0 ? _a : err.toString()));
        }
    }
    async loadInbox() {
        this.settled = false;
    ...
    createSocket() {
        this.websocket = new WebSocket(`${Config_1.default.WS_EP}`, {
            headers: {
                "User-Agent": Config_1.default.USER_AGENT,
        ...
        this.websocket.on("open", () => {
            this.websocket.send(JSON.stringify({ cmd: "sub", channel: this.username }));
        });
    }
    rejectPromise(error) {
        if (!this.settled) {
